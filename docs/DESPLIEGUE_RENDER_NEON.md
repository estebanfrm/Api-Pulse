...
**Estado:** procedimiento preparado en T06, ejecutado y verificado en T07 el 2026-09-20. [Sitio](https://api-pulse-web.onrender.com) y [API](https://api-pulse-api.onrender.com) están publicados con Neon Free; la [validación T07](VALIDACION_T07.md) registra resultados y limitaciones. El procedimiento de abajo queda para reproducción/recuperación, no como indicación de crear recursos duplicados. La decisión D09 autoriza solo planes gratuitos, Virginia/N. Virginia y subdominios incluidos, sin compra ni subida de plan.

## Topología y límites

- `render.yaml` define `api-pulse-api`: Web Service Python Free en Virginia, Uvicorn con un worker, y `api-pulse-web`: Static Site con el `dist` compilado de Vue. `autoDeployTrigger: off` conserva el control manual de la primera publicación. Ambos servicios fijan `branch: main`: sin esa clave cada servicio se queda anclado a la rama desde la que se creó el Blueprint, y un despliegue manual vuelve a publicar ese código antiguo sin avisar. **Un cambio de `branch` en `render.yaml` solo surte efecto cuando Render sincroniza el Blueprint**; si un servicio ya quedó anclado a otra rama, hay que corregirlo también en Settings del servicio antes de desplegar. Render asigna los subdominios HTTPS reales; no asumirlos a partir de los nombres.
...
- La CI añade un smoke con PostgreSQL 16 efímero: startup, `/ready`, check y persistencia tras recrear el cliente. Esto comprueba PostgreSQL/psycopg, **no Neon ni el proxy real**. T07 verificó ambos en la demo pública y registró versiones, URL, fecha, resultados y ruta de rollback en [VALIDACION_T07.md](VALIDACION_T07.md); no se probó un rollback real.
