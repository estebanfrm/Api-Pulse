...
Concurrencia, fuzzing y cadena de suministro. Siete defectos más corregidos: dos excepciones no controladas que devolvían HTTP 500 en modo local (`socket.getaddrinfo` lanza `UnicodeError` con etiquetas DNS vacías o de más de 63 caracteres; `urlsplit` lanza `ValueError` con una autoridad malformada como `http://[`), cuatro condiciones de carrera del frontend por respuestas fuera de orden —la más visible dejaba la insignia en **Offline** con el backend disponible tras pulsar «Retry connection» durante un arranque en frío— y un workflow de CI sin `permissions`. El fuzzing de 25 760 combinaciones quedó en cero respuestas 5xx. `starlette==0.41.3`, anclada por `fastapi==0.115.6`, arrastra siete avisos GHSA (tres HIGH): se comprobó uno por uno que **ninguno era explotable aquí**. El usuario decidió actualizar: `requirements.txt` pasa a `fastapi==0.141.1` (`starlette 1.6.0`, `anyio 4.15.1`, sin avisos) sin tocar código de aplicación, y la CI gana un job `Python audit` con `pip-audit`, comprobado que discrimina entre el anclaje viejo y el nuevo. Ver [estado T10](ESTADO_ACTUAL.md#t10--segunda-pasada-concurrencia-fuzzing-y-dependencias).

## T13 — Verificación final antes del portafolio

Batería completa en los dos entornos: 39 comprobaciones de API aprobadas en la demo publicada y 39 en el stack local de integración, más recorridos de interfaz en escritorio y móvil. El smoke de PostgreSQL se ejecutó por primera vez en local y aprobó. Cinco defectos corregidos en el [PR #8](https://github.com/estebanfrm/Api-Pulse/pull/8) (merge `adf5bf7`), el principal: el `Dockerfile` del frontend no copiaba `productionBuild.js`, así que `docker compose up` dejaba la interfaz caída desde T06; la CI ahora construye esa imagen y carga la configuración de Vite dentro. Se observó por primera vez la poda de 24 horas en Neon y se rehicieron las capturas del README. Ver [estado T13](ESTADO_ACTUAL.md#t13--verificación-final-antes-del-portafolio).

## Siguiente acción si se retoma

**No queda pendiente para publicar.** La demo sirve `adf5bf7`, verificada en T13, y las capturas del README son de la versión desplegada. Como seguimiento opcional: conceder acceso al repositorio a la aplicación de Render en GitHub (hoy clona por ser público, pero no recibe avisos de push), vigilar cuotas gratuitas, medir el aislamiento de cuotas entre visitantes detrás del proxy y añadir migraciones explícitas antes de cambiar columnas. No habilitar destinos arbitrarios ni servicios pagados sin decisión nueva.
